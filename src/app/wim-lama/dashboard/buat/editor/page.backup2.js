    ...
    if (slug) {
      const loadData = async () => {
        ...
        if (inv) {
          ...
          if (tName) {
            try {
              const res = await fetch(`/demo/${tName}/schema.json`);
              if (res.ok) {
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(4px)', padding: '0 10px' }}>
          <div style={{ background: 'var(--bg-secondary)', width: '100%', maxWidth: 640, borderTopLeftRadius: 20, borderTopRightRadius: 20, overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '90%', boxShadow: '0 -10px 40px rgba(0,0,0,0.3)', animation: 'slideUp 0.3s ease-out' }}>
            ...
            <div style={{ flex: 1, padding: 20, overflowY: 'auto', background: 'var(--bg-secondary)', minHeight: 0 }}>
              ...
              {/* Photo Toggle */}
              {activeModalPage.photo && (
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginBottom: 20 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: '0.85rem', color: 'var(--text-primary)', fontWeight: 600 }}>
                    <input 
                      type="checkbox" 
                      checked={!!usePhotos[activeModalPage.photo.name]} 
                      onChange={(e) => setUsePhotos(p => ({ ...p, [activeModalPage.photo.name]: e.target.checked }))} 
                      style={{ width: 16, height: 16, accentColor: 'var(--accent)' }}
                    />
                    {activeModalPage.photo.label}
                  </label>
                  
                  {usePhotos[activeModalPage.photo.name] ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12 }}>
                      <div style={{ position: 'relative', width: 80, height: 80, borderRadius: 8, border: '2px dashed var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                        {getPreviewUrl(activeModalPage.photo.name) ? (
                          <>
                            <img src={getPreviewUrl(activeModalPage.photo.name)} alt="Foto" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            <button type="button" onClick={() => {
                              setImages(p => { const n = {...p}; delete n[activeModalPage.photo.name]; return n; });
                              setExistingImages(p => { const n = {...p}; delete n[activeModalPage.photo.name]; return n; });
                        ...
                        {!getPreviewUrl(activeModalPage.photo.name) && <input type="file" accept="image/*" name={activeModalPage.photo.name} onChange={handleImageChange} style={{ opacity: 0, position: 'absolute', inset: 0, cursor: 'pointer' }} />}
                      </div>
                    ...
                    <div style={{ marginTop: 8, fontSize: '0.75rem', color: 'var(--text-muted)', fontStyle: 'italic', display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      {activeModalPage.photo.defaultText} akan digunakan.
                    </div>
                  )}
                </div>
              )}
              ...
              <div style={{ display: 'grid', gap: 16 }}>
                {activeModalPage.fields.map(f => {
                  if (f.type === 'separator') {
                    return <div key={f.name} style={{ borderBottom: '1px dashed var(--border)', margin: '16px 0 8px 0', position: 'relative' }}><span style={{ position: 'absolute', top: -10, left: 10, background: 'var(--bg-secondary)', padding: '0 8px', fontSize: '0.75rem', color: 'var(--accent)', fontWeight: 700, fontFamily: 'var(--font-outfit)' }}>{f.label}</span></div>;
                  }
